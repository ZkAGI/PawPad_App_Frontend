...
export default function ChainSelection() {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0B1426', padding: '60px 24px 40px' }}>
      ...
      <p style={{ color: '#6B7280', fontSize: 15, marginBottom: 32 }}>Select how you want to secure your wallet</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 32 }}>
        {walletTypes.map((t) => (
          <button key={t.id} onClick={() => !loading && setSelected(t.id)} style={{ padding: 20, borderRadius: 16, textAlign: 'left', width: '100%', backgroundColor: selected === t.id ? `${t.color}10` : '#111B2E', border: selected === t.id ? `2px solid ${t.color}` : '2px solid #1E3A5F', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.6 : 1 }}>
            ...
            {t.features.map((f, i) => <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}><span style={{ color: t.color, fontSize: 12 }}>✓</span><span style={{ color: '#6B7280', fontSize: 13 }}>{f}</span></div>)}
            {t.id === 'tee' && <div style={{ display: 'flex', gap: 8, marginTop: 12 }}><span style={{ backgroundColor: '#9945FF20', color: '#9945FF', fontSize: 11, padding: '4px 8px', borderRadius: 6, fontWeight: 600 }}>Solana</span><span style={{ backgroundColor: '#627EEA20', color: '#627EEA', fontSize: 11, padding: '4px 8px', borderRadius: 6, fontWeight: 600 }}>EVM</span></div>}
          </button>
        ))}
      </div>
      <button onClick={handleContinue} disabled={!selected || loading} style={{ width: '100%', padding: 16, borderRadius: 12, border: 'none', backgroundColor: selected && !loading ? '#4F7FFF' : '#1E3A5F', color: selected && !loading ? '#FFF' : '#4A5568', fontSize: 16, fontWeight: 600, cursor: selected && !loading ? 'pointer' : 'not-allowed' }}>
        {loading ? '⏳ Creating TEE wallet...' : selected === 'tee' ? 'Create TEE Wallet' : 'Continue'}
      </button>
      <button onClick={() => navigate(-1)} style={{ width: '100%', background: 'none', border: 'none', color: '#6B7280', marginTop: 16, fontSize: 14, cursor: 'pointer' }}>← Back</button>
    </div>
  );
}
